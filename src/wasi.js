...
Module['instantiateWasi'] = function (buffer, imports) {
...
function input_callback() {
    ...
    if (typeof stdin.next !== 'function'
        && typeof stdin[Symbol.iterator] === 'function') {
        stdin = stdin[Symbol.iterator]();
        Module['currentWasi'].imports.stdin = stdin;
    } else if (typeof stdin.next !== 'function') {
        console.warn('unknown stdin format');
        return null;
    }
...
Module['preRun'] = [
    () => {
        FS.init(input_callback, output_callback, err_callback);
    },
];
